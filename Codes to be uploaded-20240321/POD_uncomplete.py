...
# TO BE COMPLETED ...
np.random.seed(28)
i, j, k = np.random.randint(NN, size=3)
X = Usnap[:, i]
Y = Usnap[:, j]
Z = Usnap[:, k]
fig = plt.figure()
...
ax.scatter(X,Y,Z, label = 'Courbe', c='red')
plt.title('The manifold')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
plt.show()
...
# TO BE COMPLETED ...
# Assuming Usnap has been transposed to be NN x M as mentioned
C = Usnap.T @ A_NN1 @ Usnap  # General form
...
# TO BE COMPLETED ...
# Compute the left singular vectors (or POD modes)
modes = Usnap @ eigenvectors
# Normalize by Square Root of Eigenvalues
modes = modes / np.sqrt(eigenvalues)

# Normalization of the eigenvalues 
# TO BE COMPLETED ...
# Normalize eigenvalues to represent the fraction of the total variance
eigenvalues_normalized = eigenvalues / np.sum(eigenvalues)


# Plot of the eigenvalues
decay = np.arange(len(eigenvalues_normalized))
fig = plt.figure()
ax = fig.gca() 
ax.plot(decay, abs(eigenvalues_normalized), label='Eigenvalues',color='r') 
...
# Plot in bar chart
width = 0.5
p =plt.bar(decay,eigenvalues, width, color='b')
...
Nrb = energy_number(epsilon_POD,eigenvalues_normalized)
...
for m in range(M):
    # Retrieve the FE solution for the nth parameter
    Uh = Usnap[:, m]

    # Project the FE solution onto the reduced basis to get coefficients
    #The L2 projection
    #Urb = Brb @ (Brb.T @ Uh)
    Urb = Brb @ (Brb.T @ (A_NN1 @ Uh))

    # Calculate the difference between the FE solution and its projection
    diff = Uh - Urb

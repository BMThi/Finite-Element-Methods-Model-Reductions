...
# Define the initial guess 'u0' from 'u0_mu_exp'
u0 = interpolate(u0_mu_exp, V)

# Define the diffusivity coefficient 'mu' as a function of 'u0'
mu0 = mu(u0)
#mu0 = mu( interpolate(u0_mu_exp,V) )

print("HERE YOU ARE. END.")
#quit()

# A semi-linearized pb
if advection:
        F0 = inner(mu0 * grad(u), grad(v)) * dx + inner(vel, grad(u)) * v * dx - fp * v * dx
        ...
        F0 = inner(mu0 * grad(u), grad(v)) * dx - f * v * dx
...
while (error>eps_du and i<i_max): 
    ...
    mu_n = mu(un)  # Linearize mu at u_k
    F = mu_n * inner(grad(u), grad(v)) * dx + dot(vel, grad(u)) * v * dx - fp * v * dx
    
    # Solve the linearized problem
    solve(F == 0, un, bc)
    
    # Compute error for convergence check
    error = errornorm(u, un, 'L2')
...
# Visualization
plt.figure()
p = plot(u, title='Semi-linearized solution')
p.set_cmap("rainbow")
plt.colorbar(p)
plt.show()
